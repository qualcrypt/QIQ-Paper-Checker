/**
 * The key-scheduling Groq proxy, as middleware and as an in-process function.
 *
 * This module holds the logic that used to live inside vite.config.js, moved
 * here so the production backend (backend/) can use the same scheduling without
 * importing a Vite config. The Vite plugin is now a thin wrapper, and
 * `scheduleChat` exposes the same behaviour to server code that wants a
 * response body rather than an HTTP round-trip.
 *
 * Keys never leave this process. Nothing here logs or returns a key value.
 */

import { estimateCost } from "./keypool.js";
...
/**
 * One chat completion, scheduled across the key pool.
 *
 ...
 *
 * @param {import("./keypool.js").KeyPool} pool
 * @param {Buffer|string} body  the raw JSON request body
 * @returns {Promise<{status: number, text?: string, headers?: Headers|null,
 *   keyIndex?: number, cost: number, waited: number,
 *   proxyError?: {message: string, code: string}}>}
 *   `proxyError` is set for failures raised by the proxy itself (no keys, body
 *   too large, upstream unreachable, pool exhausted); otherwise `status`/`text`
 *   are the upstream's own answer.
 */
export async function scheduleChat(pool, body, { maxWaitMs = MAX_WAIT_MS, maxAttempts = MAX_ATTEMPTS } = {}) {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    try {
      ...
      return {
        ...
        cost,
        waited,
        proxyError: { message: `Could not reach the Groq API: ${e.message}`, code: "upstream_unreachable" },
      };
    }
  ...
  if (!lastStatus) {
    return {
      ...
      waited,
      proxyError: {
        message: `All ${pool.size} keys are at their per-minute token limit. Waiting for a refill timed out.`,
        code: "rate_limit_exceeded",
      },
    };
  }

  return { status: lastStatus, text: lastText, headers: lastHeaders, keyIndex: usedKey, cost, waited };
}

/**
 * The connect/Express middleware pair behind /api/groq. Works unchanged under
 * the Vite dev server, `vite preview`, and the production Express app.
 */
export function createGroqHandlers(pool, opts = {}) {
  const handler = async (req, res, next) => {
    ...
    const out = await scheduleChat(pool, body, opts);

    res.statusCode = out.status;
    // Index and cost only — never a key value.
    res.setHeader("x-qiq-key", `${out.keyIndex || 0}/${pool.size}`);
    res.setHeader("x-qiq-cost", String(out.cost));
    if (out.waited) res.setHeader("x-qiq-waited-ms", String(out.waited));
    ...
    if (out.proxyError) return res.end(errorBody(out.proxyError.message, out.proxyError.code));
    res.end(out.text);
  };
  ...
  return { statsHandler, handler };
}

/**
 * The Vite plugin wrapper. The heavy lifting lives in createGroqHandlers();
 * this only registers the two middlewares with a Vite dev/preview server.
 */
export function groqProxy(pool, opts) {
  const { statsHandler, handler } = createGroqHandlers(pool, opts);
  return {
    name: "qiq-groq-proxy",
